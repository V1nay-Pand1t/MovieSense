...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('/api/auth/jwt/login/', form, { withCredentials: true });
            ...
            <div className="flex items-center justify-center flex-1 ">
                <div className="bg-gray-800 p-6 rounded-lg shadow-md w-full max-w-md black">
                    <form onSubmit={handleSubmit} className="bg-gray-800 p-6 rounded-lg shadow-md">
                        ...
                        <button type="submit" className="bg-yellow-400 text-gray-800 px-4 py-2 rounded">Login</button>
                    </form>
                </div>
            </div>
        </div>
